    ...
    def _merge_shared(self, config: Dict[str, Any]) -> None:
        ...
        for param in params:
            if param in shared_params and param in new_shared_params:
                shared_params[param]["algorithms"] = new_shared_params[param][
                    "algorithms"
                ]
                shared_params[param]["search_space"] = new_shared_params[param][
                    "search_space"
                ]
    ...
    def _merge_optimized(self, config: Dict[str, Any]) -> None:
        ...
        for param in params:
            ...
            if param in new_optim_params:
                ...
                if (
                    isinstance(new_param_config, dict)
                    and "MANUAL" in new_param_config.values()
                ):
                    if param in optim_params and isinstance(optim_params[param], dict):
                        warnings.warn(
                            f"{self.OPTIMIZED_KEY}: Found 'MANUAL' marker for parameter {param}. "
                        ...
                        for algo in new_param_config:
                            if (
                                new_param_config[algo] == "MANUAL"
                                and algo not in param_config
                            ):
                                to_change_algos.append(algo)
                        for algo in to_change_algos:
                            param_config[algo] = new_param_config[algo]
                            warnings.warn(
                                f"{self.OPTIMIZED_KEY}: Found 'MANUAL' marker for parameter {param} and "
                                f"algorithm {algo}. Please set value(s) after the generation step manually!"
                            )
                        continue
                    else:
                        warnings.warn(
                            f"{self.OPTIMIZED_KEY}: Found 'MANUAL' marker for parameter {param}. Please "
