...
from timeeval import TimeEval, DatasetManager, MultiDatasetManager, TrainingType
...
def main():
    ...
    datasets += dm.select(collection="SWaT")
    datasets += dm.select(collection="WADI")
    ...
    # add the new datasets as well
    datasets += dm.select(collection="WADI")
    datasets += dm.select(collection="SWaT")
    # add the GutenTAG datasets (includes the correlation anomalies)
    datasets += dm.select(collection="GutenTAG")
    print(f"Selecting {len(datasets)} datasets")

    algorithms = [
        Baselines.deviating_from_mean(),
        Baselines.deviating_from_median()
    ]
