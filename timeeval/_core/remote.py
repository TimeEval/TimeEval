...
class Remote:
    def __init__(
        ...
        self.log.debug(
            f"Remoting configuration: {self.config}\n"
        ...
        if len(failed_on) != 0:
            raise ValueError(
                f"Could not create target folder for Dask logfile on hosts {', '.join(failed_on)}!"
            )
    ...
    def start_or_restart_cluster(self, n: int = 0) -> SpecCluster:
        ...
        if n >= 5:
            raise RuntimeError(
                "Could not start an SSHCluster because there is already one running, "
        ...
        try:
            ...
            if "Worker failed to start" in str(e):
                ...
                self.log.warning(
                    f"Failed to start cluster, because address already in use! "
    ...
    def run_on_scheduler(
        ...
        for task, args, kwargs in tqdm.tqdm(
            tasks, desc=msg, disable=self.disable_progress_bar or not progress
        ):
            self.log.debug(
                f"({msg})Running task '{task}' with args {args} and kwargs {kwargs}"
            )
    ...
    def run_on_all_hosts(
        ...
        self.log.debug(
            f"Running {len(tasks)} tasks on all cluster nodes and waiting for results"
        )
        for task, args, kwargs in tqdm.tqdm(
            tasks, desc=msg, disable=self.disable_progress_bar or not progress
        ):
    ...
    async def async_gather(
        client: Client, futures: List[Future[Dict[str, Any]]]
    ) -> None:
    ...
    def fetch_results(self) -> None:
        ...
        self.log.debug(
            f"Waiting for the results of {n_experiments} tasks submitted previously to the cluster"
        )
        coroutine_future = run_coroutine_threadsafe(
            self.async_gather(self.client, self.futures), get_event_loop()
        )
        progress_bar = tqdm.trange(
            n_experiments,
            desc="Evaluating distributedly",
            position=0,
    ...
    def close(self) -> None:
        ...
        self.log.debug(
            "Renaming dask logfile based on the hostname to allow syncing it!"
        )
