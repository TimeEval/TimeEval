...
async def _start_postgres_container(
    scheduler: Optional[Scheduler] = None, password: str = "postgres", port: int = 5432
) -> None:
    import docker
...
def _start_dashboard_container(
    ...
    storage: str = "postgresql://postgres:postgres@localhost:5432/postgres",
) -> None:
    import docker
...
def _stop_containers(
    scheduler: Optional[Scheduler] = None, remove: bool = False
) -> None:
    import docker
...
def _check_docker_available(reason: str) -> None:
    if reason:
        try:
            ...
            raise ValueError(
                f"No docker client found, but docker is required to {reason}!"
            ) from e
...
class OptunaModule(TimeEvalModule):
    ...
    def __init__(self, config: OptunaConfiguration):
        ...
        if self.config.default_storage is None:
            ...
            isinstance(self.config.default_storage, str)
            and self.config.default_storage == "postgresql"
        ):
            and_required = " and " if check_docker else ""
            check_docker += (
                and_required + f"start the Optuna storage {self.config.default_storage}"
            )
    ...
    def prepare(self, timeeval: TimeEval) -> None:
        ...
        if (
            ...
            log.debug(
                f"starting managed postgresql storage backend at ({storage_url})..."
            )
            self.storage_url = storage_url

            tasks.append(
                (_start_postgres_container, [], {"password": password, "port": port})
            )

        elif (
            isinstance(self.config.default_storage, str)
            and self.config.default_storage == "journal-file"
        ):
            journal_file_path = str(timeeval.results_path / "optuna-journal.log")
            self.config.default_storage = lambda: JournalStorage(
                JournalFileStorage(
                    journal_file_path, lock_obj=JournalFileOpenLock(journal_file_path)
                )
            )
        ...
        if timeeval.distributed:
            log.debug(
                f"distributed mode: starting {len(tasks)} containers on scheduler"
            )
            timeeval.remote.run_on_scheduler(
                tasks, msg="Starting Optuna containers on scheduler"
            )
        else:
            ...
            timeeval.remote.run_on_scheduler(
                [
                    (
                        _stop_containers,
                        [],
                        {"remove": self.config.remove_managed_containers},
                    )
                ],
