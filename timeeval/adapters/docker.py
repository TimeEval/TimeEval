...
class DockerAdapter(Adapter):
    ...
    def __init__(
        ...
        image_name: str,
        tag: str = "latest",
        group_privileges: str = "akita",
        skip_pull: bool = False,
        timeout: Optional[Duration] = None,
        memory_limit_overwrite: Optional[int] = None,
    ...
    def _get_gid(group: str) -> str:
        ...
        return subprocess.run(
            CMD % group, capture_output=True, text=True, shell=True
        ).stdout.strip()
    ...
    def _get_compute_limits(self, args: Dict[str, Any]) -> Tuple[int, float]:
        limits: Tuple[int, float] = args.get(
            "resource_constraints", ResourceConstraints()
        ).get_compute_resource_limits(
            memory_overwrite=self.memory_limit, cpu_overwrite=self.cpu_limit
        )
        return limits
    ...
    def _should_use_prelim_model(args: Dict[str, Any]) -> bool:
        ...
        constraints = args.get("resource_constraints", ResourceConstraints())
        result: bool = (
            exec_type == ExecutionType.TRAIN or exec_type == ExecutionType.TRAIN.value
        ) and constraints.use_preliminary_model_on_train_timeout
        return result
    ...
    def _should_use_prelim_results(args: Dict[str, Any]) -> bool:
        ...
        constraints = args.get("resource_constraints", ResourceConstraints())
        result: bool = (
            exec_type == ExecutionType.EXECUTE
            or exec_type == ExecutionType.EXECUTE.value
        ) and constraints.use_preliminary_scores_on_execute_timeout
        return result
    ...
    def _run_container(self, dataset_path: Path, args: Dict[str, Any]) -> Container:
        ...
        print(
            f"Running container '{self.image_name}:{self.tag}' with env='{repr(env_vars)}' in {algorithm_interface.executionType} mode."
        )
        ...
        try:
            return client.containers.run(
                ...
                volumes={
                    str(dataset_path.parent.resolve()): {
                        "bind": str(DATASET_TARGET_PATH),
                        "mode": "ro",
                    },
                    str(self._results_path(args, absolute=True)): {
                        "bind": str(RESULTS_TARGET_PATH),
                        "mode": "rw",
                    },
                },
            ...
            print(
                f"Could not start Docker container for algorithm ({self.image_name}:{self.tag}:"
            )
    ...
    def _run_until_timeout(self, container: Container, args: Dict[str, Any]) -> None:
            ...
            if "timed out" in str(e):
                if self._should_use_prelim_results(args):
                    ...
                    if (self._results_path(args) / SCORES_FILE_NAME).is_file():
                        print(
                            f"Container timeout after {timeout}, but TimeEval disregards this because "
                        ...
                        print(
                            f"Container timeout after {timeout} and "
                        ...
                        raise DockerTimeoutError(
                            f"{self.image_name} could not create results after {timeout}"
                        ) from e
                    ...
                    if (self._results_path(args) / MODEL_FILE_NAME).is_file():
                        ...
                        print(
                            f"Container timeout after {timeout} and 'ResourceConstraints.use_preliminary_model_on_train_timeout' is "
                            "set to True. However, the algorithm did not store a model; "
                            "raising DockerTimeoutError anyway!"
                        )
                        raise DockerTimeoutError(
                            f"{self.image_name} could not build a model within {timeout}"
                        ) from e
                else:
                    print(
                    ...
                    raise DockerTimeoutError(
                        f"{self.image_name} timed out after {timeout}"
                    ) from e
            else:
        ...
        if result["StatusCode"] == 137:
            print(
                f"Docker algorithm ran out of memory (status {result['StatusCode']})!"
            )
            raise DockerMemoryError(
                f"Docker algorithm exceeded memory limit of {self._get_compute_limits(args)[0]} Bytes!"
            )

        elif result["StatusCode"] != 0:
            print(
                f"Docker algorithm failed with status code '{result['StatusCode']}', consider container logs above."
            )
            raise DockerAlgorithmFailedError(
                f"Status '{result['StatusCode']}', please consider log files in {self._results_path(args, absolute=True)}!"
            )
