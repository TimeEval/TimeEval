...
class ResultMerger:
    ...
    def _append(
        self, df_ref: pd.DataFrame, record: pd.Series, change_filesystem: bool = False
    ) -> pd.DataFrame:
        new_idx = len(df_ref)
        self._logger.info(
            f"Found a run not contained in reference experiment; adding it as {new_idx}."
        )
    ...
    def _replace(
        ...
        idx: int,
        record: pd.Series,
        change_filesystem: bool = False,
    ) -> None:
        self._logger.info(
            f"Found replacement for reference run {idx} (status={df_ref.loc[idx, 'status']}) "
    ...
    def prepare_target_folder(self) -> None:
        if self.right_folder == self.target_folder or (
            self.left_folder == self.target_folder and not self.force_inplace
        ):
            raise ValueError(
                "The merge result can only be stored in a new folder for safety reasons!"
            )
        ...
        if not self.force_inplace:
            if self.target_folder.exists():
                if not list(self.target_folder.iterdir()):
                    self._logger.debug(
                        "Target folder exists and is empty. Removing it in order to copy the reference "
            ...
            self._logger.info(
                "Populating target folder with results from left (reference experiment)"
            )
            ...
            if self.left_folder != self.target_folder:
                raise ValueError(
                    "If --force-inplace is given, you must also set the target folder to the reference "
            ...
            self._logger.info(
                f"Using left (reference experiment) folder ({self.left_folder}) as target folder"
            )
            self.target_folder = self.left_folder

    def merge_runs(
        self, change_filesystem: bool = False
    ) -> Tuple[pd.DataFrame, List[int]]:
        self._logger.debug(
            f"Changing filesystem is {'enabled' if change_filesystem else 'disabled'}!"
        )
        ...
        for i, record in df_right.iterrows():
            ...
            if len(ref_indices) == 0:
                ...
                if ref_status == "Status.ERROR" and (
                    ...
                    or right_status == "Status.OK"
                    or right_status == "Status.TIMEOUT"
                ):
                ...
                self._logger.warning(
                    f"There are ambiguous matched for reference indices {ref_indices} and right "
    ...
    def copy_misc_files(self) -> None:
        misc_files = [
            f
            for f in self.right_folder.iterdir()
            if f.is_file() and f.name != "results.csv"
        ]
        for file in misc_files:
            ...
            while target_filename.exists():
                counter += 1
                target_filename = (
                    self.target_folder / f"{file.stem}-{counter}{file.suffix}"
                )

            self._logger.info(
                f"Copying additional file {file.name} --> {target_filename.name}"
            )
    ...
    def merge(self):
        ...
        self._logger.info(
            f"Added {len(changed_idxs)} runs from {self.right_folder} to {self.left_folder}."
...
def _create_arg_parser() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Merge results of two different experiments together."
    )
    parser.add_argument(
        "left",
        type=Path,
    ...
    parser.add_argument(
        "target", type=Path, help="Path where the merged results are written to."
    )
    parser.add_argument(
        "--loglevel",
        default="INFO",
...
if __name__ == "__main__":
    ...
    ResultMerger(
        args.left, args.right, args.target, args.force_inplace, args.overwrite_errors
    ).merge()
