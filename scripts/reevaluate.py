...
class Evaluator:
    ...
    def __init__(
        self, results_path: Path, data_paths: List[Path], metrics: List[Metric]
    ):
        ...
        self._logger.warning(
            f"The Evaluator changes the results folder ({self.results_path}) in-place! "
    ...
    def evaluate(self, select_index: Optional[Path], evaluate_successful: bool = False):
        ...
        self._logger.info(
            f"Re-evaluating {len(exp_indices)} experiments from {len(self.df)} experiments of "
        ...
        for i in exp_indices:
            ...
            if not evaluate_successful and s_exp.status == "Status.OK":
                self._logger.info(
                    f"Exp-{i:06d}: Skipping, because experiment was successful."
                )
                continue
            ...
            if not docker_scores_path.exists() or not params_path.exists():
                self._logger.error(
                    f"Exp-{i:06d}: Experiment ({s_exp.algorithm}-{s_exp.collection}-{s_exp.dataset}) "
            ...
            if not evaluate_successful and processed_scores_path.exists():
                self._logger.debug(
                    f"Exp-{i:06d}: Skipping reprocessing of anomaly scores, they are present."
                )
                ...
                self._logger.info(
                    f"Exp-{i:06d}: Writing anomaly scores to {processed_scores_path}."
                )
            ...
            if not evaluate_successful and all(
                m.name in metric_scores for m in self.metrics
            ):
                self._logger.debug(
                ...
                for metric_name in self.metrics:
                    try:
                        ...
                        self._logger.warning(
                            f"Exp-{i:06d}: Exception while computing metric {metric_name}!",
                            exc_info=e,
                        )
                        errors += 1
                        continue
                ...
                if metric_scores:
                    self._logger.info(
                        f"Exp-{i:06d}: Writing new metrics to {metrics_path}!"
                    )
            ...
            if metric_scores and errors == 0:
                self._logger.debug(
                    f"Exp-{i:06d}: Updating status to success (Status.OK)."
                )
                s_update = s_exp.copy()
