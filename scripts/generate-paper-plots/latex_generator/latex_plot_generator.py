...
from .config import algo_cite_lut


class LatexPlotGenerator:

    def __init__(self, result_file: Path, dominant_quality_metric: str = "ROC_AUC", aggregation: str = "mean") -> None:
        self.dominant_quality_metric = dominant_quality_metric
        self.dominant_aggregation = aggregation
        ...
        self.df_asl = None
        self.df_asl = self._extract_asl(self.df, dominant_quality_metric, aggregation)

    def _extract_asl(self, df: pd.DataFrame,
                     quality_metric: str = "ROC_AUC",
                     aggregation: str = "mean") -> pd.DataFrame:
        index_columns = ["algo_input_dimensionality", "algo_training_type", "algorithm"]

        if (quality_metric == self.dominant_quality_metric
                and aggregation == "mean"
                and self.df_asl is not None
                and df.equals(self.df)):
            return self.df_asl
        ...
        df_asl[aggregation] = df_asl.agg(aggregation, axis=1)
        df_asl = df_asl.reset_index().sort_values(by=index_columns[:-1] + [aggregation],
                                                  ascending=False).set_index(index_columns)
        df_asl = df_asl.drop(columns=aggregation)
        return df_asl

    def _extract_overview_table(self, relative_rates: bool = True) -> pd.DataFrame:
        df_algorithm_error_counts = self.df.pivot_table(index=["algorithm"], columns=["error_category"],
                                                        values="repetition", aggfunc="count")
    ...
    @staticmethod
    def _adapt_levels_and_add_rules(algorithms: List[dict],
                                    attr: str = "dim",
                                    rulespan: str = "1-6",
                                    limit_factor: int = 2):
    ...
    def generate_overview_table(self, target: Union[str, Path],
                                relative_rates: bool = True,
                                metrics: List[str] = ["ROC_AUC"],
                                include_gutentag_only: bool = False) -> None:
        # Prepare target
        target_path = Path(target).resolve()
        print(f"Making sure that the target path ({target_path}) is writable")
        target_path.parent.mkdir(parents=True, exist_ok=True)

        print("Configuration:")
        print(f"  sorting by metric={self.dominant_quality_metric}, agg={self.dominant_aggregation}")
        print(f"  relative errors={relative_rates}")
        print(f"  include GutenTAG only boxplots={include_gutentag_only}")
        print(f"  boxplot metrics: {metrics}")
        ...
        for metric in metrics:
            asls[metric] = self._extract_asl(self.df, quality_metric=metric, aggregation="mean")
        if include_gutentag_only:
            asls["gutentag"] = self._extract_asl(self.df[self.df["collection"] == "GutenTAG"], quality_metric="ROC_AUC",
                                                 aggregation="mean")
        ...
        for _, s in df_overview_table.iterrows():
            ...
            if include_gutentag_only:
                try:
                    values = asls["gutentag"].loc[(s["dimensionality"], s["learning type"], s["algorithm"])]
                    values = values.dropna().values
                    gt_boxplot = self._compute_boxplot_values(s["algorithm"], values)
                except Exception:
                    # ignore all exceptions and just skip this boxplot
                    pass
            algorithms.append({
                "name": algo_cite_lut[s["algorithm"]],
                "timeout": format(s["# TIMEOUT"]),
        ...
        # Write latex using template
        print(f"Writing overview table latex code to {target_path}")
        with target_path.open("w") as fh:
            fh.write(file_template.render(
                algorithms=algorithms,
...
def main():
    gen = LatexPlotGenerator(Path("./paper-plots-results.csv"))
