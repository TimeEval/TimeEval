...
def _create_arg_parser():
    ...
    parser.add_argument(
        "input_file",
        type=str,
        help="Path to input file with the anomaly scores (point-based)",
    )
    parser.add_argument(
        "label_file",
        type=str,
        help="Path to the label file (looks for 'is_anomaly' or "
        "'AnomalyLabel' column, otherwise takes the last column)",
    )
    parser.add_argument(
        "--metric",
        type=str,
        default=Metric.ROC_AUC.name,
        choices=[
            Metric.ROC_AUC.name,
            Metric.PR_AUC.name,
    ...
    parser.add_argument(
        "-p", "--plot", action="store_true", help="Enable plotting the metric curve"
    )
    parser.add_argument(
        "--save-plot", action="store_true", help="Save the metric plot as pdf"
    )
...
if __name__ == "__main__":
    ...
    result = Metric[args.metric](
        anomaly_labels, anomaly_scores, plot=args.plot, plot_store=args.save_plot
    )
