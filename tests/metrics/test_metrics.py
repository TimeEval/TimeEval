...
class TestMetrics(unittest.TestCase):
    ...
    def test_range_based_f_score_thresholding(self):
        ...
        y_true = np.array([0, 1, 0, 0])
        result = RangeFScore(thresholding_strategy=FixedValueThresholding(), beta=1)(
            y_true, y_score
        )
        self.assertAlmostEqual(result, 0.66666, places=4)
    ...
    def test_range_based_precision_thresholding(self):
        y_score = np.array([0.1, 0.9, 0.8, 0.2])
        y_true = np.array([0, 1, 0, 0])
        result = RangePrecision(thresholding_strategy=FixedValueThresholding())(
            y_true, y_score
        )
    ...
    def test_range_based_recall_thresholding(self):
        y_score = np.array([0.1, 0.9, 0.8, 0.2])
        y_true = np.array([0, 1, 0, 0])
        result = RangeRecall(thresholding_strategy=FixedValueThresholding())(
            y_true, y_score
        )
        self.assertEqual(result, 1)
    ...
    def test_precision_at_k(self):
        y_pred = np.array([0, 0.1, 1.0, 0.6, 0.1, 0, 0.4, 0.5])
    ...
    def test_edge_cases(self):
        ...
        with warnings.catch_warnings():
            ...
            for m in pr_metrics:
                ...
                self.assertTrue(
                    score <= 0.2,
                    msg=f"{m.name}(y_true, y_inverted)={score} is not <= 0.2",
                )
    ...
    def test_f_score_thresholding(self):
        y_score = np.array([0.1, 0.9, 0.8, 0.2])
        y_true = np.array([0, 1, 0, 0])
        result = F1Score(thresholding_strategy=FixedValueThresholding())(
            y_true, y_score
        )
        self.assertAlmostEqual(result, 0.66666, places=4)
    ...
    def test_precision_thresholding(self):
        y_score = np.array([0.1, 0.9, 0.8, 0.2])
        y_true = np.array([0, 1, 0, 0])
        result = Precision(thresholding_strategy=FixedValueThresholding())(
            y_true, y_score
        )
        self.assertAlmostEqual(result, 0.5, places=4)
