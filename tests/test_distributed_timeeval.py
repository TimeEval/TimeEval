...
class TestDistributedTimeEval(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.test_config = RemoteConfiguration(
            ...
            kwargs_overwrites={
                ...
                "worker_options": {
                    "preload": f"\"import sys; sys.path.insert(0, '{os.getcwd()}')\""
                }
            },
        )
    ...
    def test_distributed_results_and_shutdown_cluster(self):
        ...
        datasets = DatasetManager(
            "./tests/example_data", custom_datasets_file=datasets_config
        )

        with tempfile.TemporaryDirectory() as tmp_path:
            timeeval = TimeEval(
                datasets,
                list(zip(cycle(["custom"]), self.results.dataset.unique())),
                self.algorithms,
                distributed=True,
            ...
            results = results.groupby(by=["algorithm", "collection", "dataset"])[
                "ROC_AUC"
            ].mean()
            results = pd.DataFrame(results).reset_index()
            pd.testing.assert_frame_equal(results, self.results.loc[:, compare_columns])
            ...
            self.assertFalse(
                any(
                    ("distributed.cli.dask_worker" in c)
                    or ("distributed.cli.dask_scheduler" in c)
                    for p in processes
                    for c in p.cmdline()
                ),
    ...
    def test_distributed_phases(
        self, mock_cluster, mock_client, mock_docker, mock_call, mock_popen
    ):
        ...
        with tempfile.TemporaryDirectory() as tmp_path:
            timeeval = TimeEval(
                ...
                remote_config=RemoteConfiguration(
                    scheduler_host="test-host", worker_hosts=["test-host2"]
                ),
                results_path=Path(tmp_path),
                n_jobs=1,
            )
    ...
    def test_aliases_excluded(
        ...
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            timeeval = TimeEval(
                ...
                distributed=True,
                remote_config=RemoteConfiguration(
                    scheduler_host="localhost", worker_hosts=hosts
                ),
                results_path=Path(tmp_path),
                n_jobs=1,
            )
            ...
            self.assertTrue(
                rsync.params[0], ["rsync", "-a", f"test-host:{tmp_path}/", tmp_path]
            )
    ...
    @patch("timeeval._core.remote.Client")
    @patch("timeeval._core.remote.SSHCluster")
    def test_catches_future_exception(
        self, mock_cluster, mock_client, mock_docker, mock_call, mock_popen
    ):
        ...
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            timeeval.run()
            self.assertListEqual(
                timeeval.results[["status", "error_message"]].values[0].tolist(),
                [Status.ERROR, "ExceptionForTest('test-exception')"],
            )
    ...
    @patch("timeeval._core.remote.Client")
    @patch("timeeval._core.remote.SSHCluster")
    def test_catches_future_timeout_exception(
        self, mock_cluster, mock_client, mock_docker, mock_call, mock_popen
    ):
    ...
    @patch("timeeval._core.remote.Client")
    @patch("timeeval._core.remote.SSHCluster")
    def test_catches_future_memory_exception(
        self, mock_cluster, mock_client, mock_docker, mock_call, mock_popen
    ):
    ...
    def test_catches_future_exception_dask(self):
        datasets = DatasetManager("./tests/example_data")
        algo = Algorithm(
            name="docker-test-timeout",
            main=DockerAdapter(
                TEST_DOCKER_IMAGE, skip_pull=True, timeout=Duration("5 seconds")
            ),
            data_as_file=True,
    ...
    def test_catches_future_timeout_exception_dask(self):
        ...
        algo = Algorithm(
            name="docker-test-timeout",
            main=DockerAdapter(
                TEST_DOCKER_IMAGE, skip_pull=True, timeout=Duration("1 seconds")
            ),
            data_as_file=True,
        )
