...
class TestOutputData(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.algorithms = [
            Algorithm(
                ...
                main=FunctionAdapter(deviating_from_mean),
                param_config=self.hyper_params,
            )
        ]
    ...
    def test_output_files_exists(self):
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            timeeval = TimeEval(
                self.datasets, [self.DATASET], self.algorithms, results_path=tmp_path
            )
            timeeval.run()
            parent_path = (
                tmp_path
    ...
    def test_log_exists_and_is_correct(self):
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            timeeval = TimeEval(
                self.datasets, [self.DATASET], [algo], results_path=tmp_path
            )
            timeeval.run()
            parent_path = (
                tmp_path
    ...
    def test_results_csv_exists_and_is_correct(self):
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            self.assertEqual(
                str(results.hyper_params_id.values[0]), hash_dict({"a": 0})
            )
