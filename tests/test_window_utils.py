...
class TestReverseWindowing(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.y_mean = np.concatenate(
            [np.arange(1, 3.5, 0.5), np.arange(4, 198), np.arange(198, 200.5, 0.5)]
        )
        self.y_mean_non_reversed = np.arange(3, 199)
        self.y_median = np.concatenate(
            [np.arange(1, 3.5, 0.5), np.arange(4, 198), np.arange(198, 200.5, 0.5)]
        )
        self.y_sum = np.concatenate(
            [
                np.array([1, 3, 6, 10]),
                np.arange(15, 199 * 5, 5),
                np.array([794, 597, 399, 200]),
            ]
        )
    ...
    @pytest.mark.skipif(
        condition=os.getenv("CI", "false") == "true",
        reason="CI never finishes on sopedu",
    )
    ...
    def test_mp_windowing_chunks_mean(self):
        y_reversed = ReverseWindowing(
            window_size=5, n_jobs=4, chunksize=10
        ).fit_transform(self.X)
        np.testing.assert_array_equal(self.y_mean, y_reversed)
