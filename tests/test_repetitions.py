...
from tests.fixtures.algorithms import (
    DeviatingFromMean,
    DeviatingFromMedian,
    ErroneousAlgorithm,
)
...
class TestRepetitions(unittest.TestCase):
    ...
    def test_multiple_results(self):
        ...
        np.testing.assert_array_almost_equal(
            results["ROC_AUC_mean"].values, self.results["ROC_AUC"].values
        )
    ...
    def test_return_no_aggregation(self):
        ...
        self.assertEqual(
            len(timeeval.exps.datasets) * len(self.algorithms) * 3, len(results)
        )
    ...
    def test_return_aggregated_but_not_short(self):
        ...
        self.assertEqual(
            len(timeeval.exps.datasets) * len(self.algorithms), len(results)
        )
    ...
    def test_error_in_repetition(self):
        ...
        algorithms = [
            ...
            Algorithm(
                ...
                main=ErroneousAlgorithm(raise_after_n_calls=5),
            ),
            Algorithm(name="deviating_from_median", main=DeviatingFromMedian()),
        ]
        with tempfile.TemporaryDirectory() as tmp_path:
