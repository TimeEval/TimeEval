...
def generates_results(dataset, from_file: bool = False) -> pd.DataFrame:
    ...
    datasets = DatasetManager(
        "./tests/example_data", custom_datasets_file=CUSTOM_DATASET_PATH
    )

    with tempfile.TemporaryDirectory() as tmp_path:
        timeeval = TimeEval(
            datasets, [dataset], algorithms, results_path=Path(tmp_path)
        )
        timeeval.run()
...
def generates_results_multi(dataset) -> pd.DataFrame:
    ...
    with tempfile.TemporaryDirectory() as tmp_file:
        timeeval = TimeEval(
            datasets, [dataset], algorithms, results_path=Path(tmp_file)
        )
        timeeval.run()
    return timeeval.results
...
class TestImportData(unittest.TestCase):
    ...
    def test_generates_correct_results(self):
        ...
        np.testing.assert_array_equal(
            generated_results[self.KEYS].values, true_results[self.KEYS].values
        )
